...
@login_required
def bet_search():
    return render_template("bets/search.html", find_results = Bet.find_bet_results(str(current_user.id), '1'), teams = Team.query.all(), form = BetSearchForm())
...
def bet_search_results():
    ...
    if d == 'correct':
        return render_template("bets/search.html", find_results = Bet.find_bet_results(str(current_user.id), '1'), teams = Team.query.all(), form = BetSearchForm())
    if d == 'failed':
        return render_template("bets/search.html", find_results = Bet.find_bet_results(str(current_user.id), '2'), teams = Team.query.all(), form = BetSearchForm())
    if d == 'void':
        return render_template("bets/search.html", find_results = Bet.find_bet_results(str(current_user.id), '3'), teams = Team.query.all(), form = BetSearchForm())
